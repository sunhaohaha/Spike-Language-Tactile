...
def run_modality_experiment(
    ...
    modality_names = list(selected_modalities)

    fusion_name = "+".join(modality_names)
    folder_name = "_".join(modality_names)

    print("\n" + "=" * 90)
    print(f"Start three-modal fusion: {fusion_name}")
    print("=" * 90)
    ...
    fusion_info = {
        ...
        "selected_modalities": fusion_name,
...
output_root = f"trimodal_fusion_results_{timestamp}"
...
modality_combinations = [
    ("temperature", "weight", "materials"),
    ("temperature", "weight", "shape"),
    ("temperature", "materials", "shape"),
    ("weight", "materials", "shape")
]

print("\nModality combinations to run:")
...
for combo in modality_combinations:
    result = run_modality_experiment(
        selected_modalities=combo,
        all_data=all_data,
...
summary_path = os.path.join(output_root, "all_trimodal_fusion_summary.xlsx")
summary_df.to_excel(summary_path, index=False)

print("\n" + "=" * 90)
print("Three-modal fusion experiments finished.")
